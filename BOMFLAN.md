...
- Preview configuration is now saved: dedicated admin username and encrypted password, encrypted Meta token, verified dataset ID, Test Events code, Graph v25.0, action source chat, and live sending disabled. Secrets were entered by the owner and not read or committed. A new preview deployment is required to apply them.
- Production remains commit `fbff160`; the existing Oflan Worker has not been deployed or edited.
- The owner opened Bom Flan Dataset in Chrome profile Imas (BomFlanProfile); supplied Events Manager and Business Settings screenshots confirm dataset 1626416872401155 is owned by Bom Flan, and provide its Test Events code. The other Chrome profile lacks access. Token presence is confirmed encrypted in Cloudflare, but API acceptance and receipt in Events Manager still need verification. No test or live event has been sent to Meta yet.
...
For WhatsApp ad attribution with `business_messaging`, the form requires the **original** `ctwa_clid` from the WhatsApp referral, plus the server's WABA ID. This implementation does not capture WhatsApp webhooks automatically. Do not invent click IDs or claim that a phone-only manual event optimizes WhatsApp purchases. `chat` is a separate manual-chat reporting path, not an automatic fallback to messaging optimization. Confirm Meta's current accepted payload and eligibility in the connected account before activation. On 23 September 2026, Meta’s official [version table](https://developers.facebook.com/docs/graph-api/changelog/versions/) confirmed Graph v25.0 is supported until 29 July 2028. Preview is configured with v25.0.

The owner confirmed use of the ordinary WhatsApp Business phone app, with manual order entry. Preview therefore uses `chat`, which Meta’s [server-event documentation](https://developers.facebook.com/documentation/ads-commerce/conversions-api/parameters/server-event) defines for messaging-app conversions. This tests manual purchase reporting; it does not establish click-to-WhatsApp attribution or eligibility for purchase-through-messaging optimization. That separate route needs genuine referral identifiers and verified WhatsApp integration.
